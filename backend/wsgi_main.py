...
from agent_integration import create_agent_endpoint, list_agents_endpoint, chat_with_agent_endpoint
...
def application(environ, start_response):
    ...
    if method == 'GET':
        if path == '/api/agents':
            return list_agents_endpoint(environ, start_response, '')
        elif path == '/':
            response = {
            ...
            response = {"models": models, "recommended": "llama3.3:70b"}
            status = '200 OK'
            headers = [
                ('Content-Type', 'application/json'),
                ('Access-Control-Allow-Origin', '*')
...
<body>
    <div class="container">
        ...
        <div class="endpoint">
            <div class="method">GET /api/agents</div>
            <p>Lista degli agenti attivi.</p>
        </div>
        <div class="endpoint">
            <div class="method">POST /api/agents/create</div>
            <p>Crea un nuovo agente.</p>
        </div>
        <div class="endpoint">
            <div class="method">POST /api/agents/chat</div>
            <p>Chatta con un agente specifico.</p>
        </div>
    </div>
</body>
</html>
"""
    ...
    if method == 'POST':
        ...
        if path == '/api/agents/create':
            return create_agent_endpoint(environ, start_response, body)
        elif path == '/api/agents/chat':
            return chat_with_agent_endpoint(environ, start_response, body)
        elif path == '/api/chat':
